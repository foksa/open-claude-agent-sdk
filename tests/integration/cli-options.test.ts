...
testWithBothSDKs(
  ...
  async (sdk: SDKType) => {
    // Use isolated temp dir per sdk to avoid cross-test interference
    const tempCwd = mkdtempSync(join(tmpdir(), `sdk-test-continue-${sdk}-`));
    ...
    // Second query: use continue to resume and ask for the token
    ...
    console.log(`   [${sdk}] continue — resumed session with token recall`);
  },
  120000
);
