...
async function testSystemPrompt(name: string, args: string[]) {
  ...
  for await (const msg of query({
    ...
    if (msg.type === 'result') {
      ...
      if (response.toLowerCase().includes('javascript') ||
...
async function main() {
  ...
  await testSystemPrompt(
    'Test 1: Default (loads CLAUDE.md)',
    []
  );
  ...
  await testSystemPrompt(
    'Test 3: --system-prompt (replaces default)',
  ...
  await testSystemPrompt(
    'Test 5: --append-system-prompt (adds to default)',
    ['--append-system-prompt', 'Always be extra concise.']
  );
