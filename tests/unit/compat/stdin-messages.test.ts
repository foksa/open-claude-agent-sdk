...
describe('stdin message compatibility', () => {
  ...
  test.concurrent(
    'all 18 hook events register correctly matching official SDK',
    async () => {
      // Every hook event from HOOK_EVENTS array in SDK 0.2.50
      const allHookEvents = [
        'PreToolUse',
        'PostToolUse',
        'PostToolUseFailure',
      ...
      console.log('   All 18 hook events register correctly');
    },
    { timeout: 60000 }
  );
